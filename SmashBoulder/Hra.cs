...
namespace CSHra
{
    ...
    class Hrdina : PohyblivyPrvek
    {
        ...
        public override void UdelejKrok()
        {
            ...
            else if (mapa.JeBalvan(nove_x, y))
            {
                if (mapa.JeVolno(nove2_x, y))
                {                    
                    mapa.Presun(nove_x, y, nove2_x, y);
                    mapa.Presun(x, y, nove_x, nove_y);
                }
            }
            else if (mapa.JeDiamant(nove_x, nove_y))
            {
                mapa.SeberDiamant(nove_x, nove_y);
                mapa.Presun(x, y, nove_x, nove_y);
            }
            else if (mapa.JeOtevrenyVychod(nove_x, nove_y))
            {
                mapa.Presun(x, y, nove_x, nove_y);
    ...
    class Prisera : PohyblivyPrvek
    {
        ...
        public override void UdelejKrok()
        {
            ...
            // ###########################################################
            if (mapa.JeVolno(this.x, nove_y))
            {
                mapa.Presun(this.x, this.y, this.x, nove_y);
            }
            // ###########################################################
        }
