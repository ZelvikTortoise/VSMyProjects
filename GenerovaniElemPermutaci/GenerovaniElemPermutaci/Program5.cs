...
{
    // TODO: REDUCABLE() -> USE AS REDUCABLE TO BREAK (START IN SHORTLEX); CAN CREATE REDUCE() AS COPY OF REDUCABLE()
    class Perm
    {
        ...
        public static int GetNumberOfPermsFound()
        {
            return perms.Count;            
        }
        ...
        public static void CountPermsByInvs(out int[] permsByInvs)
        {
            permsByInvs = new int[Program.MaxInvs + 1];
            for (int i = 0; i < Perm.perms.Count; i++)
            {
                permsByInvs[Perm.perms[i].Length]++;
            }
        }

        public static bool IsPermsSymmetric()
        {
            bool symmetric = true;
            int maxIndex = Program.MaxInvs / 2; // = 15: 0, 15; 1, 14; ...; 7, 8 => until 7; or = 6: 0, 6; 1, 5; ...; 3, 3
            CountPermsByInvs(out int[] sortedByInvs);
            for (int i = 0, j = sortedByInvs.Length - 1; i <= maxIndex; i++, j--)
            {
                if (sortedByInvs[i] != sortedByInvs[j])
                {
                    symmetric = false;
                    break;
                }
        ...
        public static void ShowPerms(bool onePerLine)
        {
            ...
            if (onePerLine)
            {
                ...
                for (int i = 1; i < Perm.perms.Count; i++)
                {
                    if (Perm.perms[i - 1].Length / 10 != Perm.perms[i].Length / 10)
                    {
            ...
            Console.WriteLine();
            if (!Perm.IsPermsSymmetric())
            {
                Console.WriteLine("NENÍ SYMETRICKÉ!");
            }
            else
            {
                Console.WriteLine("JE SYMETRICKÉ. :-)");
            }
        }
        public static void PrintPerms(TextWriter writer, bool onePerLine)
        {
            ...
            using (writer)
            {
                ...
                if (!Perm.IsPermsSymmetric())
                {
                    writer.WriteLine("NENÍ SYMETRICKÉ!");
                }
                else
                {
                    writer.WriteLine("JE SYMETRICKÉ. :-)");
                }
            }
        }
    }
    ...
    class Program
    {
        ...
        static int[] RandomElps(int[] elps, Random random)
        {
            for (int i = 0; i < elps.Length; i++)
            {
                elps[i] = random.Next(MaxElp + 1);
            }
        ...
        static void Generate()
        {
            ...
            Perm perm;
            while (Perm.GetNumberOfPermsFound() < maxPerms)
            {
                perm = new Perm(RandomElps(elps, random));
                Perm.TryAddToPerms(perm);
            }
        }
