...
namespace Share
{
    public partial class Form2 : Form
    {
        ...
        // Proměnné rychlosti:
        int speedStartingInterval = 4000;  // Jaký je počáteční interval nyan cat. Mění se s obtížností.
        ...
        {
            this.Location = new Point(475, 125);    // Sjednotit s this.Location pro Form2Úvod!
            switch (Difficulty)
            {
                case "easy":
        ...
        {
            string pom = direction;
            direction = "none";     // Aby Nyan cat nenarážela (zastaví se).
        ...
        public void Magnet(int x, int y)
        {
            ...
            if ((y + foodImage.Height) <= pictureBoxNyanCat.Location.Y)
            {
                y += stepMagnet;
            }
        ...
        public void Food()
        {
            ...
            do
            {
                foodX = random.Next(0, 461);
                foodY = random.Next(0, 461);
            }
        ...
        public void HamPower()
        {
            if (((pictureBoxNyanCat.Location.X + pictureBoxNyanCat.Width) > pictureBoxPower.Location.X && pictureBoxNyanCat.Location.X < (pictureBoxPower.Location.X + pictureBoxPower.Width)) && ((pictureBoxNyanCat.Location.Y + pictureBoxNyanCat.Height) > pictureBoxPower.Location.Y && pictureBoxNyanCat.Location.Y < (pictureBoxPower.Location.Y + pictureBoxPower.Height)))      // zkontrolovat a opravit
            {
                ...
                if (pictureBoxPower.Image == magnet)
                {
                    ...
                    labelPowerTakenPopis.Text = "Magnetic food";
                ...
                else if (pictureBoxPower.Image == borders)
                {
                    ...
                    labelPowerTakenPopis.Text = "Opened borders";
                ...
                else if (pictureBoxPower.Image == reversed)
                {
                    ...
                    labelPowerTakenPopis.Text = "Reversed control";
                ...
                else if (pictureBoxPower.Image == swap)
                {
                    ...
                    labelPowerTakenPopis.Text = "Swap";
                    points += 2 * pointFoodGain;    // *2.0
                    labelSkóre.Text = points.ToString();
                    timerDuration.Interval = 1000;
                    progressBar.Value = 0;

                    int newSnakeX = pictureBoxFood.Location.X;
                    int newSnakeY = pictureBoxFood.Location.Y;
        ...
        public void Kontrola()
        {
            if (!powerUpBorders)
            {
                if (pictureBoxNyanCat.Location.X < -2 || pictureBoxNyanCat.Location.X > 448 || pictureBoxNyanCat.Location.Y < -2 || pictureBoxNyanCat.Location.Y > 448)
                {
                    ...
                    DialogResult result = MessageBox.Show("                                     PROHRÁLI JSTE. \n Nyan cat narazila do stěny. Chcete hru spustit od začátku? \n\n Poznámka: Vaše skóre touto cestou nebude uloženo.", "Kočka na měkko", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (result == DialogResult.Yes)
                    {
                        Restart();
                    }
                    ...
                    {
                        Form2ScoreSave save = new Form2ScoreSave();
                        this.Hide();
                        save.Score = labelSkóre.Text + " points";
                        save.Dif = Difficulty;
            ...
            else
            {
                ...
                if (pictureBoxNyanCat.Location.X > 473)
                {
                ...
                if (pictureBoxNyanCat.Location.Y > 473)
                {
        ...
        private void timerSpeed_Tick(object sender, EventArgs e)
        {
            ...
            {
                if (timerSnake.Interval > (50 + speedChangeInterval))
                {
        ...
        private void timerPowerSpawn_Tick(object sender, EventArgs e)
        {
            if (powerUpSpawnDelay == 0)
            {
                ...
                {
                    powerUpSpawnDelay = random.Next(1, powerUpMaxSpawnTime - powerUpMinSpawnTime + 1);     // od 0 do 15 sekund delaye.
                }
            }
        ...
        private void timerPowerDespawn_Tick(object sender, EventArgs e)
        {
            if (!powerTaken)
            {
                ...
                if (gameTimePower >= (powerUpDespawnTime - powerUpBlinkTime))       // zkusit! Má tam být jen >? (myslím, že ne)
                {
                    gameTimePower = 0;
                    timerPowerDespawn.Enabled = false;
        ...
        private void timerDuration_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (pictureBoxNyanCat.Location.X < -27)
                    {
                        pictureBoxNyanCat.Location = new Point(23, pictureBoxNyanCat.Location.Y);
                        direction = "right";
                        zleva = false;
                    }
                    else if ((pictureBoxNyanCat.Location.X + pictureBoxNyanCat.Width) > 473)
                    {
                    ...
                    if (pictureBoxNyanCat.Location.Y < -27)
                    {
                        pictureBoxNyanCat.Location = new Point(pictureBoxNyanCat.Location.X, 23);
                        direction = "down";
                    }
                    else if ((pictureBoxNyanCat.Location.Y + pictureBoxNyanCat.Height) > 473)
                    {
